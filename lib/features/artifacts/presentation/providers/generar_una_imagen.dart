...
final generarUnaImagenProvider =
    Provider<Future<LoQueSalioDelDibujo> Function(ElEncargo)>((ref) {
      return (encargo) async {
        ...
        if (!hecha.salio) {
          return NoSePudoDibujar(hecha.problema);
        }
        ...
        try {
          ...
          return NoSePudoDibujar('no se pudo guardar: ${e.message}');
        }

        // Para que la lista de documentos la vea sin reabrir nada.
        ref.invalidate(artifactsProvider);
        return LaImagenSalio(ruta: destino.path, id: hecha.id);
      };
    });
